#! /usr/bin/env python3
# ^ Thats called a shebang ^
# "#!" is the shebang itself, tells the interpreter to run the following code
...
class MinimalPyPublisher(Node):
  ...
  def __init__(self):
    ...
    self.publisher_1 = self.create_publisher(String, '/py_example_topic', 10) # create_publisher method is part of Node class

    # Create a timer with a period of 0.5 seconds to trigger publishing of message
    timer_period = 0.5 # in seconds
    self.timer = self.create_timer(timer_period, self.timer_callback)

    # Initialize a counter variable for message content [not necessary, just for learning purposes in this case]
    self.i = 0
