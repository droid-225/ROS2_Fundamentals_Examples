 ...
 *  TestMessageContent: Vefifies published message format
 * 
 * @author Rishit Shah
...
#define TESTING_EXCLUDE_MAIN // Prevents the main function in the implementation file from being included
...
TEST_F(TestMinimalPublisher, TestNodeCreation)
{
  ...
  EXPECT_EQ(pub_endpoints.size(), 1u); // Makes sure node has only one publisher on the topic
}
